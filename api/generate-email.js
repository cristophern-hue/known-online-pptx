...
async function generatePptx(DATA) {
  ...
  // Top 3 campaigns by apertura (parse percentage for sorting)
  const parseRate = str => parseNum((str || "0").replace("%", "").replace("pp", ""));
  const campanas  = Array.isArray(DATA.EMAIL_CAMPANAS) ? DATA.EMAIL_CAMPANAS : [];
  const top3      = [...campanas]
    .sort((a, b) => parseRate(b.apertura) - parseRate(a.apertura))
    .slice(0, 3);
  ...
  // ── SLIDE 2 – RESUMEN PLATAFORMA ──────────────────────────────────────────
  {
    ...
    const cardW = cols <= 4 ? 2.1 : 1.75;
    const totalW = cols * cardW + (cols - 1) * 0.22;
  ...
  if (campanas.length > 0) {
    ...
    s.addText([
      ...
      { text: `${DATA.PLATAFORMA_EMAIL || "Email"} – ${DATA.PERIODO_ACTUAL_LABEL || ""}`, options: { bold: true, color: ORANGE, fontSize: 26, fontFace: "Trebuchet MS" } },
    ...
    // Build column config based on whether ecommerce data exists
    const cols = hasEcommerce
      ? [
          { hdr: "Campaña",   key: "nombre",        w: 2.55, align: "left",   color: ORANGE },
      ...
      : [
          { hdr: "Campaña",  key: "nombre",   w: 3.5,  align: "left",   color: ORANGE },
    ...
    const summaryY = tY + 0.34 + maxRows * 0.33 + 0.1;
    if (summaryY < 5.2) {
      const summaryKpis = [
        { label: "Total envíos",    val: DATA.EMAIL_ENVIOS    || "" },
        { label: "Apertura prom.",  val: DATA.EMAIL_APERTURA  || "" },
        { label: "CTOR prom.",      val: DATA.EMAIL_CTOR      || "" },
        { label: "Bajas prom.",     val: DATA.EMAIL_BAJAS     || "" },
      ];
      if (DATA.EMAIL_TRANSACCIONES) summaryKpis.push({ label: "Transacciones", val: DATA.EMAIL_TRANSACCIONES });
      if (DATA.EMAIL_INGRESOS)      summaryKpis.push({ label: "Ingresos",       val: DATA.EMAIL_INGRESOS });
      const kW = 1.55;
      const skStart = (10 - summaryKpis.length * kW) / 2;
      summaryKpis.forEach((k, i) => {
        const kx = skStart + i * kW;
        s.addText(k.label, { x: kx, y: summaryY,        w: kW, h: 0.2,  fontSize: 8, color: GRAY_TEXT, fontFace: "DM Sans", align: "center" });
        s.addText(k.val,   { x: kx, y: summaryY + 0.2,  w: kW, h: 0.32, fontSize: 12, bold: true, color: DARK, fontFace: "DM Sans", align: "center" });
      });
    }
  }

  // ── SLIDE 4 – TOP 3 CAMPAÑAS POR APERTURA ────────────────────────────────
  if (top3.length > 0) {
    const s = pres.addSlide();
    s.background = { color: WHITE };

    s.addText("Top 3 Campañas", { x: 0.5, y: 0.22, w: 6, h: 0.55, fontSize: 28, bold: true, color: DARK, fontFace: "Trebuchet MS" });
    s.addText("por tasa de apertura", { x: 0.5, y: 0.78, w: 6, h: 0.3, fontSize: 13, color: ORANGE, fontFace: "DM Sans", bold: true });
    ...
    top3.forEach((c, i) => {
      ...
      s.addShape(pres.shapes.RECTANGLE, { x, y, w: cardW, h: 0.08, fill: { color: ORANGE }, line: { color: ORANGE } });

      // Medal + rank
      s.addText(`${medals[i]}  #${i + 1}`, { x, y: y + 0.15, w: cardW, h: 0.3, fontSize: 11, bold: true, color: ORANGE, fontFace: "DM Sans", align: "center" });

      // Campaign name (2 lines max)
      ...
      // Big apertura number
      s.addShape(pres.shapes.RECTANGLE, { x: x + 0.35, y: y + 1.2, w: cardW - 0.7, h: 0.9, fill: { color: ORANGE }, line: { color: ORANGE } });
      ...
      s.addText("apertura", { x: x + 0.35, y: y + 2.12, w: cardW - 0.7, h: 0.22, fontSize: 9, color: ORANGE, fontFace: "DM Sans", align: "center" });

      // Detail metrics
      const details = [
        { lbl: "Envíos", val: c.envios   || "—" },
        { lbl: "CTOR",   val: c.ctor     || "—" },
        { lbl: "Bajas",  val: c.bajas    || "—" },
      ];
  ...
  // ── SLIDE 5 – GA4 CANAL EMAIL (condicional) ───────────────────────────────
  if (hasGA4) {
    const s = pres.addSlide();
    s.background = { color: WHITE };
    ...
    const ga4StartX = 0.4;
    ga4Kpis.forEach((k, i) => {
      const x = ga4StartX + i * (ga4CardW + 0.22);
      const y = 1.2;
