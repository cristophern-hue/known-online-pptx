...
async function generatePptx(DATA) {
  ...
  const kpis = [
    ...
    { label: "Leads totales",   val: parseNum(DATA.LEADS_TOTAL) > 0 ? DATA.LEADS_TOTAL : (isManar ? DATA.META_LEADS || "" : ""),  delta: parseNum(DATA.LEADS_TOTAL) > 0 ? DATA.LEADS_DELTA || "" : (isManar ? DATA.META_LEADS_DELTA || "" : ""), note: `${DATA.PERIODO_ANTERIOR_LABEL || "Año ant."}: ${parseNum(DATA.LEADS_TOTAL) > 0 ? DATA.LEADS_PREV || "" : (isManar ? DATA.META_LEADS_PREV || "" : "")}`, up: parseNum(DATA.LEADS_TOTAL) > 0 ? DATA.LEADS_DELTA_UP === true : (isManar && DATA.META_LEADS_DELTA_UP === true) },
    { label: "CPL promedio",    val: parseNum(DATA.CPL_TOTAL) > 0 ? DATA.CPL_TOTAL : (isManar ? DATA.META_CPL || "" : ""),  delta: parseNum(DATA.CPL_TOTAL) > 0 ? DATA.CPL_DELTA || "" : (isManar ? DATA.META_CPL_DELTA || "" : ""), note: `${DATA.PERIODO_ANTERIOR_LABEL || "Año ant."}: ${parseNum(DATA.CPL_TOTAL) > 0 ? DATA.CPL_PREV || "" : (isManar ? DATA.META_CPL_PREV || "" : "")}`, up: parseNum(DATA.CPL_TOTAL) > 0 ? DATA.CPL_DELTA_UP === true : (isManar && DATA.META_CPL_DELTA_UP === true) },
