...
async function generatePptx(DATA) {
  ...
  // ── Colors & period labels ────────────────────────────────────────────────
  const { ORANGE, ORANGE2, DARK, WHITE, LIGHT_BG, GRAY_TEXT, LIGHT_GRAY, GREEN, GREEN_BG, RED, RED_BG, AMBER, AMBER_BG, BLUE, LIGHT_BLUE } = COLORS;
  ...
  if (DATA.ATIKA_PINTEREST_INV) {
    ...
    sAtika.addText("Performance General", { x: 0.4, y: 0.17, w: 9.2, h: 0.42, fontSize: 26, bold: true, color: DARK, fontFace: "DM Sans" });
  ...
  const _impA = parseNum(DATA.META_IMPRESIONES) + parseNum(DATA.GOOGLE_IMPRESIONES);
  const _impP = parseNum(DATA.META_IMPRESIONES_PREV) + parseNum(DATA.GOOGLE_IMPRESIONES_PREV);
  const _impDeltaUp  = _impA >= _impP;
  const _impDeltaStr = _impP > 0 ? ((_impDeltaUp ? "+" : "") + ((_impA - _impP) / _impP * 100).toFixed(1).replace(".", ",") + "%") : "";
  const fmtNumCompact = n => {
    if (n >= 1_000_000) return `${n / 1_000_000 < 10 ? (n / 1_000_000).toFixed(1).replace(".", ",") : Math.round(n / 1_000_000)} M`;
    if (n >= 1_000)     return `${n / 1_000     < 10 ? (n / 1_000    ).toFixed(1).replace(".", ",") : Math.round(n / 1_000    )} K`;
  ...
  s2.addText(isTiendaInglesa ? "Inversión · Impresiones · Clicks  ·  Meta Ads + Google Ads" : "Inversión · Revenue · ROAS  ·  Meta Ads + Google Ads", { x: 0.5, y: 0.78, w: 9, h: 0.3, fontSize: 13, color: GRAY_TEXT, fontFace: "DM Sans" });
  ...
  const _invDeltaUp  = isAtika ? _invA >= _invP : DATA.INVERSION_DELTA_UP === true;
  const _invDeltaStr = isAtika && _invP > 0
    ? ((_invA >= _invP ? "+" : "") + ((_invA - _invP) / _invP * 100).toFixed(1).replace(".", ",") + "%")
    : DATA.INVERSION_DELTA || "";
  ...
  const kpis = isTiendaInglesa
    ? [
        { label: "Inversión total",     val: fmtMoneyCompact(DATA.INVERSION_TOTAL), delta: _invDeltaStr,           note: `${DATA.PERIODO_ANTERIOR_LABEL || "Período ant."}: ${DATA.INVERSION_PREV || ""}`,  up: _invDeltaUp },
        { label: "Impresiones totales", val: fmtNumCompact(_impA),                  delta: _impDeltaStr,           note: `${DATA.PERIODO_ANTERIOR_LABEL || "Período ant."}: ${fmtNumCompact(_impP)}`,        up: _impDeltaUp },
        { label: "Clicks totales",      val: DATA.CLICKS_TOTAL || "",               delta: DATA.CLICKS_DELTA || "", note: `${DATA.PERIODO_ANTERIOR_LABEL || "Período ant."}: ${DATA.CLICKS_PREV || ""}`,      up: DATA.CLICKS_DELTA_UP === true },
      ]
    : [
  ...
  const kpiCardW = isTiendaInglesa ? 2.7  : 2.1;
  const kpiStep  = isTiendaInglesa ? 3.25 : 2.32;
  kpis.forEach((k, i) => {
    const x  = 0.4 + i * kpiStep;
    const dX = x + (kpiCardW - 0.9) / 2;
    s2.addShape(pres.shapes.RECTANGLE, { x, y: 1.2, w: kpiCardW, h: 1.55, fill: { color: LIGHT_BG }, line: { color: "F0E8E0", width: 0.5 } });
    s2.addShape(pres.shapes.RECTANGLE, { x, y: 1.2, w: kpiCardW, h: 0.07, fill: { color: ORANGE }, line: { color: ORANGE } });
    s2.addText(k.label, { x, y: 1.32, w: kpiCardW, h: 0.3,  fontSize: 10, color: GRAY_TEXT, fontFace: "DM Sans", align: "center" });
    s2.addText(k.val,   { x, y: 1.62, w: kpiCardW, h: 0.52, fontSize: 26, bold: true, color: DARK, fontFace: "DM Sans", align: "center" });
    s2.addShape(pres.shapes.RECTANGLE, { x: dX, y: 2.17, w: 0.9, h: 0.27, fill: { color: k.up ? GREEN_BG : RED_BG }, line: { color: k.up ? GREEN_BG : RED_BG } });
    s2.addText(k.delta, { x: dX, y: 2.17, w: 0.9, h: 0.27, fontSize: 11, bold: true, color: k.up ? GREEN : RED, fontFace: "DM Sans", align: "center" });
    s2.addText(k.note,  { x, y: 2.5,  w: kpiCardW, h: 0.25, fontSize: 9,  color: GRAY_TEXT, fontFace: "DM Sans", align: "center" });
  });
  ...
  if (DATA.CHAIDE_VENTAS_AGENTES_ACTUAL) {
    ...
    sAg.addText("Canal Agentes", { x: 0.5, y: 0.18, w: 7, h: 0.52, fontSize: 28, bold: true, color: DARK, fontFace: "DM Sans" });
    ...
    agKPIs.forEach((k, i) => {
      ...
      sAg.addText(k.val,   { x: x + 0.14, y: y + 0.52, w: 2.62, h: 0.48, fontSize: 22, bold: true, color: valColor,  fontFace: "DM Sans", align: "center" });
    });
    ...
    consolidadoCols.forEach((col, i) => {
      ...
      sAg.addText(col.total, { x: x + 2.4, y: y + 1.04, w: 1.9, h: 0.3, fontSize: 14, bold: true, color: DARK, fontFace: "DM Sans", align: "right" });
  ...
  const ga4Metrics = [
    ...
    { icon: "$", label: "Inversión publicitaria", sub: "Total Meta Ads + Google Ads",              val26: fmtMoneyCompact(DATA.INVERSION_TOTAL), val25: isAtika ? fmtMoneyCompact(_invP) : (DATA.INVERSION_PREV || ""), delta: _invDeltaStr, deltaColor: _invDeltaUp ? GREEN : RED, deltaBg: _invDeltaUp ? GREEN_BG : RED_BG },
  ...
  ga4Metrics.forEach((m, i) => {
    ...
    s7.addText(m.val26,  { x: x + 0.14, y: y + 0.92, w: 1.5,  h: 0.38, fontSize: fs7, bold: true, color: DARK, fontFace: "DM Sans" });
  ...
  } // end !isTiendaInglesa (GA4 slide)

  // ── SLIDE 3B – ECOMMERCE PLATFORM (OPCIONAL) ─────────────────────────────
  if (DATA.ECOMMERCE_INGRESOS) {
    let sEc = pres.addSlide();
    sEc.background = { color: WHITE };
    sEc.addText("Performance Ecommerce", { x: 0.5, y: 0.2, w: 7, h: 0.55, fontSize: 28, bold: true, color: DARK, fontFace: "DM Sans" });
    ...
    ecMetrics.forEach((m, i) => {
      ...
      sEc.addText(m.val,   { x: x + 0.14, y: y + 1.12, w: 2.32, h: 0.65, fontSize: fsEc, bold: true, color: DARK, fontFace: "DM Sans" });
  ...
  // ── SLIDE GA4 CAMPAÑAS (TIENDA INGLESA) – placeholder para screenshot ──────
  if (isTiendaInglesa) {
  ...
  s3.addText("Meta Ads", { x: 0.5, y: 0.22, w: 6, h: 0.52, fontSize: 28, bold: true, color: WHITE, fontFace: "DM Sans" });
  ...
  s4.addText("Google Ads", { x: 0.5, y: 0.22, w: 6, h: 0.52, fontSize: 28, bold: true, color: WHITE, fontFace: "DM Sans" });
  ...
  if (DATA.TOP_ANUNCIOS_META_TIENE_DATOS && Array.isArray(DATA.TOP_ANUNCIOS_META) && DATA.TOP_ANUNCIOS_META.length > 0) {
    ...
    s5b.addText("Top Anuncios Meta por Compras", { x: 0.5, y: 0.2, w: 7, h: 0.55, fontSize: 28, bold: true, color: DARK, fontFace: "DM Sans" });
