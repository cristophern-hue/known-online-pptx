...
const labelCortoActual =
  DATA.PERIODO_ACTUAL_SHORT || `${MESES[mesIdx]} '${String(añoActual).slice(2)}`;

const labelCortoAnterior =
  DATA.PERIODO_ANTERIOR_SHORT || `${MESES[mesIdx]} '${String(añoActual - 1).slice(2)}`;
  ...
  if (DATA.ATIKA_PINTEREST_INV) {
    ...
    const atikaRows = [
     ...
     { label: "Ventas sitio (sin canceladas)",     a: DATA.ECOMMERCE_INGRESOS || "",          p: DATA.ECOMMERCE_INGRESOS_PREV || "",      d: DATA.ECOMMERCE_INGRESOS_DELTA || "",      up: DATA.ECOMMERCE_INGRESOS_DELTA_UP === true },
